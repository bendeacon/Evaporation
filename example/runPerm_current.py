...
def compPerm(fn_conf, chem=None, sc_Kw_paras=None, sc_D_paras=None, disp=1) :
    ...
    t_start, t_end, Nsteps = [0, 3600*10, 30]
    t_range = np.linspace(t_start, t_end, Nsteps)  
    init_conc = _skin.getComp(0,0).getMeshConc()[0]
    for i in range(Nsteps-1):
        ...
        print( 'Vehicle conc =', _skin.getComp(0,0).getMeshConc()[0] ) 
        # Simulate
    ...
    return flux_sc_down / (init_conc*0.0001)
    #return flux_sc_down / _conf.init_conc_vh
    #return flux_sc_down / 0.0001
